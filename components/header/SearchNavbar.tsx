...
const SearchNavbar = () => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<BrandStatus | null>(null);
  ...
  const removeRecentSearch = (termToRemove: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const updatedSearches = recentSearches.filter(search => search.term !== termToRemove);
    setRecentSearches(updatedSearches);
    localStorage.setItem('recentSearches', JSON.stringify(updatedSearches));
  };
  ...
  return (
    <div className="relative search-container">
      <div className='flex items-center justify-center relative w-full md:w-96'>
        ...
        <Input 
          placeholder={isSearching ? "Buscando..." : "Buscar por numero de parte"} 
        ...
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 shadow-lg border dark:border-gray-700">
          {searchTerm ? (
            ...
            searchResults.map((product) => (
              ...
              >
                <div>
                  <div className="font-medium">{product.supplier_product_id}</div>
